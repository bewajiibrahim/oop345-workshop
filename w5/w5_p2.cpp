...
int main(int argc, char** argv)
{
	...
	if (argc == 5) {
		...
		do {
			...
			// if line is not empty, and there is no comment character
			if (line[0] != '#' && line != "") {
				library += line;
			}
		} while (file);
	...
	// returns a book, captures by reference

	auto priceConversion = [&](Book& book)->Book& {
		if (book.country() == "US") {
	...
	for (unsigned i{ 0 }; i < library.size(); i++) {
		library[i] = priceConversion(library[i]);
	}
